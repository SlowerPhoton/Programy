...
/*
    for(int i = 0; i < 32; i++)
        cout << '\t' << i << ": " << LR[0][16][i] << endl;
    for(int i = 0; i < 32; i++)
        cout << '\t' << i+32 << ": " << LR[1][16][i] << endl;*/
...
int main()
{
    ...
    int key[8] = {155, 100, 30, 0, 25, 222, 25, 25};
    bool binaryKey[8*8];
    toBinary(key, binaryKey, 8);

    /*for(int i = 56; i < 64; i++)
        cout << '\t' << i << ": " << binaryKey[i] << endl;*/
    ...
    for (int i = 0; i < 8; i++)
        message.push_back( (unsigned char) key[i]);

    bool binaryMessage[64];
    messageToBinary(binaryMessage, message, 0);
    bool C[64];
    encodeMessage(binaryMessage, subkeys, C);

    /*pozustatky z f()*/
    int RP[4] = {15,22,35,189};
    int KP[6] = {15,22,35,189, 255, 0};
    bool R[32];
    bool K[48];
    toBinary(RP, R, 4);
    toBinary(KP, K, 6);
...
void createSubkeys(bool binaryKey[8*8], bool subkeys[16][48]){
    ...
    /*for(int i = 0; i < 48; i++)
        cout << '\t' << i << ": " << subkeys[15][i] << endl;
    /*for(int i = 0; i < 28; i++)
        cout << '\t' << i+28 << ": " << CD[1][16][i] << endl;*/
}
...
int lookUpInSBox(int which, bool address[6], bool binaryOutcome[4]){
   ...
   { {13,  2,   8,  4,   6, 15,  11,  1,  10,  9,   3, 14,   5,  0,  12,  7,},
    ...
    row[0] = address[0]; row[1] = address[5];
                        ...
                        /*if (debug == 0){
                        /*for(int a = 0; a < 4; a++)
                            cout << '\t' << i+a << ": " << answer[a] << endl;*/
                           /* cout << toDecimal(address, 6) << endl;*/
                            //if (debug < 8)cout <<row[0] << endl << row[1] << endl;
