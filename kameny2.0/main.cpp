...
void rekurze(vetev *ukazatel, int sk){
        ...
        for(int i=0; i < kolikDat*2; i+=2){
            ...
            if(vzdalenostH!=0 && vzdalenostH < ukazatel->vzdalenost){     /*pomocne*/ /*cout << "pridavam k " << ukazatel->kam << " bod " << i/2 << ", vzdalenost je " << vzdalenostH << " v hloubce " << sk+1 << endl;*/
...
int main()
{
    ...
    vetev *pom = zacatek;
    while(pom != NULL){
        //cout << "vzdalenost (" << data[0] << ", " << data[1] << ") a (" << data[pom->kam*2] << ", " << data[pom->kam*2+1] <<  ") je " << pom -> vzdalenost << endl;
        pom = pom->next;
    }
    cout << endl;
     ...
     */
     ...
     cout << /*"nejvetsi dosazena hloubka " <<*/ maxSkoku << endl;
